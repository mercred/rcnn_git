function roidb = roidb_from_munchen(imdb)
...
try
  ...
  for i = 1:length(imdb.image_ids)
    ...
    try
      munich_rec = PASreadrecord(sprintf('/home/user/Documents/3k_Munchen/Annotations/%s.txt', imdb.image_ids{i}));
    catch
      munich_rec = [];
    end
    roidb.rois(i) = attach_proposals(munich_rec, regions.boxes{i}, imdb.class_to_id);
  end
...
% ------------------------------------------------------------------------
function rec = attach_proposals(munich_rec, boxes, class_to_id)
% ------------------------------------------------------------------------
...
%        class: [2108x1 uint8]
if isfield(munich_rec, 'objects')
  gt_boxes = cat(1, munich_rec.objects(:).bbox);
  all_boxes = cat(1, gt_boxes, boxes);
  gt_classes = class_to_id.values({munich_rec.objects(:).class});
