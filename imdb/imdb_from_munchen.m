function imdb = imdb_from_munchen(root_dir, image_set)
% imdb = imdb_from_voc(root_dir, image_set, year)
%   Builds an image database for the Munchen database located
%   at root_dir using the image_set.
%
...
%TODO: change all file names to be generated from root parameter
cache_file = ['./imdb/cache/imdb_munchen_' image_set];
try
  load(cache_file);
catch
  ...
  imdb.name = ['munchen_' image_set];
  imdb.image_dir = fileparts('/home/user/Documents/3k_Munchen/Images/%s.jpg');
  imdb.image_ids = textread(sprintf('/home/user/Documents/3k_Munchen/ImageSets/%s.txt', image_set), '%s');
  imdb.extension = 'jpg';
  imdb.classes = {['30'];['10'];['16'];['22'];['11'];['20'];['17'];['23']};
  ...
  imdb.eval_func = @imdb_eval_munchen;
  imdb.roidb_func = @roidb_from_munchen;
  imdb.image_at = @(i) ...
  ...
  for i = 1:length(imdb.image_ids)
    ...
    info = imfinfo(sprintf('/home/user/Documents/3k_Munchen/Images/%s.jpg', imdb.image_ids{i}));
